...
void memdivmer_t_init(memdivmer_t *initp)
{
 ...
 for (uint_t li = 0; li < 52; li++)
 {
  bafhlst_t_init(&initp->dm_mdmlielst[li], 3, li, (1UL << li));
 }
 bafhlst_t_init(&initp->dm_onemsalst, 1, 0, 1UL);
 return;
}
...
bool_t init_memarea_core(machbstart_t *mbsp)
{
 ...
 if (initchkadr_is_ok(mbsp, phymarea, (sizeof(memarea_t) * 4)) != 0)
 {
  return 0;
 }

 memarea_t *virmarea = (memarea_t *)phyadr_to_viradr((adr_t)phymarea);
 for (uint_t mai = 0; mai < 4; mai++)
 {

  memarea_t_init(&virmarea[mai]);
 }

 virmarea[0].ma_type = 1;
 virmarea[0].ma_logicstart = 0;
 virmarea[0].ma_logicend = (0 + 0x2000000 - 1);
 virmarea[0].ma_logicsz = 0x2000000;


 virmarea[1].ma_type = 2;
 virmarea[1].ma_logicstart = 0x2000000;
 virmarea[1].ma_logicend = (0x2000000 + (0x400000000 - 0x2000000) - 1);
 virmarea[1].ma_logicsz = (0x400000000 - 0x2000000);


 virmarea[2].ma_type = 3;
 virmarea[2].ma_logicstart = 0x400000000;
 virmarea[2].ma_logicend = (0x400000000 + (0xffffffffffffffff - 0x400000000));
 virmarea[2].ma_logicsz = (0xffffffffffffffff - 0x400000000);

 virmarea[3].ma_type = 4;
 ...
 mbsp->mb_memznnr = 4;
 mbsp->mb_memznsz = sizeof(memarea_t) * 4;


 mbsp->mb_nextwtpadr = (((phymarea + sizeof(memarea_t) * 4) + ((1UL << 12))-1) & ~(((1UL << 12))-1));
 return 1;
}
 ...
 void init_memarea()
{
 if (init_memarea_core(&kmachbsp) == 0)
 {
...
{
 if (0 == mareap || 0 == fmstat || 0 == fmsanr || 0 == retmsastatp ||
  0 == retmsaendp || 0 == retfmnr)
 {
  return 0;
 }
...
uint_t continumsadsc_is_ok(msadsc_t *prevmsa, msadsc_t *nextmsa, msadflgs_t *cmpmdfp)
{
 if (0 == prevmsa || 0 == cmpmdfp)
 {
  return (~0UL);
 }

 if (0 != prevmsa && 0 != nextmsa)
 {
  if (prevmsa->md_indxflgs.mf_marty == cmpmdfp->mf_marty &&
   0 == prevmsa->md_indxflgs.mf_uindx &&
   (0) == prevmsa->md_indxflgs.mf_mocty &&
   (0) == prevmsa->md_phyadrs.paf_alloc)
  {
   if (nextmsa->md_indxflgs.mf_marty == cmpmdfp->mf_marty &&
    0 == nextmsa->md_indxflgs.mf_uindx &&
    (0) == nextmsa->md_indxflgs.mf_mocty &&
    (0) == nextmsa->md_phyadrs.paf_alloc)
   {
    if ((nextmsa->md_phyadrs.paf_padrs << 12) - (prevmsa->md_phyadrs.paf_padrs << 12) == (1UL << 12))
    {
     return 2;
    }
    return 1;
...
bool_t scan_len_msadsc(msadsc_t *mstat, msadflgs_t *cmpmdfp, uint_t mnr, uint_t *retmnr)
{
 uint_t retclok = 0;
 uint_t retnr = 0;
 if (0 == mstat || 0 == cmpmdfp || 0 == mnr || 0 == retmnr)
 {
  return 0;
 }
...
{
 if (0 == mareap || 0 == fmstat || 0 == fntmsanr ||
  0 == fmsanr || 0 == retmsastatp || 0 == retmsaendp || 0 == retfmnr)
 {
  return 0;
 }
 if (*fntmsanr >= fmsanr)
 ...
 for (; tmidx < fmsanr; tmidx++)
 {
  ...
  if (msastat[tmidx].md_indxflgs.mf_marty == mdfp->mf_marty &&
   ...
   (0) == msastat[tmidx].md_indxflgs.mf_mocty &&
   (0) == msastat[tmidx].md_phyadrs.paf_alloc)
  {
...
uint_t merlove_setallmarflgs_onmemarea(memarea_t *mareap, msadsc_t *mstat, uint_t msanr)
{
 ...
 {
  if ((0) == mstat[mix].md_indxflgs.mf_marty)
  {

   phyadr = mstat[mix].md_phyadrs.paf_padrs << 12;

   if (phyadr >= mareap->ma_logicstart && ((phyadr + (1UL << 12)) - 1) <= mareap->ma_logicend)
   {
...
bool_t continumsadsc_add_procmareabafh(memarea_t *mareap, bafhlst_t *bafhp, msadsc_t *fstat, msadsc_t *fend, uint_t fmnr)
{
 if (0 == mareap || 0 == bafhp || 0 == fstat || 0 == fend || 0 == fmnr)
 {
  return 0;
 }
 if (1 != bafhp->af_stus || 3 != mareap->ma_type)
 {
  return 0;
 }
 ...
 {
  fstat[tmpnr].md_indxflgs.mf_olkty = (2);
...
bool_t continumsadsc_add_bafhlst(memarea_t *mareap, bafhlst_t *bafhp, msadsc_t *fstat, msadsc_t *fend, uint_t fmnr)
{
 ...
 fstat->md_indxflgs.mf_olkty = (1);

 fstat->md_odlink = fend;


 fend->md_indxflgs.mf_olkty = (2);
...
bool_t continumsadsc_mareabafh_core(memarea_t *mareap, msadsc_t **rfstat, msadsc_t **rfend, uint_t *rfmnr)
{
 if (0 == mareap || 0 == rfstat || 0 == rfend || 0 == rfmnr)
 {
  return 0;
 }
 ...
 if ((2 == bafhp->af_stus || 3 == bafhp->af_stus) && 3 != mareap->ma_type)
 {

  tmpmnr = retval - bafhp->af_oderpnr;


  if (continumsadsc_add_bafhlst(mareap, bafhp, mstat, &mstat[bafhp->af_oderpnr - 1], bafhp->af_oderpnr) == 0)
  {
   return 0;
  }
 ...
 if (1 == bafhp->af_stus && 3 == mareap->ma_type)
 {
  if (continumsadsc_add_procmareabafh(mareap, bafhp, mstat, mend, *rfmnr) == 0)
  {
   return 0;
  }
  *rfmnr = 0;
...
bool_t merlove_continumsadsc_mareabafh(memarea_t *mareap, msadsc_t *mstat, msadsc_t *mend, uint_t mnr)
{
 if (0 == mareap || 0 == mstat || 0 == mend || 0 == mnr)
 {
  return 0;
 }
 uint_t mnridx = mnr;
 ...
 for (; (mnridx > 0 && 0 != fend);)
 {
  ...
  if (continumsadsc_mareabafh_core(mareap, &fstat, &fend, &mnridx) == 0)
  {
...
bool_t merlove_mem_onmemarea(memarea_t *mareap, msadsc_t *mstat, uint_t msanr)
{
 ...
 msadsc_t *retstatmsap = 0, *retendmsap = 0, *fntmsap = mstat;
 uint_t retfindmnr = 0;
 uint_t fntmnr = 0;
 bool_t retscan = 0;
 ...
 for (; fntmnr < msanr;)
 {
  ...
  if (0 != retstatmsap && 0 != retendmsap)
  {
   ...
   if (merlove_continumsadsc_mareabafh(mareap, retstatmsap, retendmsap, retfindmnr) == 0)
   {
...
bool_t merlove_mem_core(machbstart_t *mbsp)
{
 ...
 for (uint_t maidx = 0; maidx < (uint_t)mbsp->mb_memznnr; maidx++)
 {
  ...
  if (merlove_mem_onmemarea(&marea[maidx], mstatp, msanr) == 0)
  {
   return 0;
  }
...
{
 if (0 == mstat || 0 == bafhp || 0 == mareap)
 {
  return 0;
 }
 if ((1) != mstat->md_indxflgs.mf_olkty &&
  (2) != mstat->md_indxflgs.mf_olkty)
 {
  return 0;
 }
 if (0 == mstat->md_odlink)
 {
  return 0;
 }
 ...
 if ((2) == mstat->md_indxflgs.mf_olkty)
 {
  mend = mstat;
 }
 if (0 == mend)
 ...
 if ((2) != mend->md_indxflgs.mf_olkty)
 {
  return 0;
 }
 ...
 if (mnr == 1)
 {
  ...
  if ((0) != mstat->md_phyadrs.paf_alloc ||
   0 != mstat->md_indxflgs.mf_uindx)
  {
   return 0;
  }
  phyadr = mstat->md_phyadrs.paf_padrs << 12;
  if (phyadr < mareap->ma_logicstart || (phyadr + (1UL << 12) - 1) > mareap->ma_logicend)
  {
   return 0;
  }
 ...
 for (uint_t mi = 0; mi < mnr - 1; mi++)
 {
  ...
  if ((0) != mstat[mi].md_phyadrs.paf_alloc ||
   0 != mstat[mi].md_indxflgs.mf_uindx)
  {
   return 0;
  }
  if ((0) != mstat[mi + 1].md_phyadrs.paf_alloc ||
   0 != mstat[mi + 1].md_indxflgs.mf_uindx)
  {
   return 0;
  }
  if (((mstat[mi].md_phyadrs.paf_padrs << 12) + (1UL << 12)) != (mstat[mi + 1].md_phyadrs.paf_padrs << 12))
  {
   return 0;
  }
  if ((mstat[mi].md_phyadrs.paf_padrs << 12) < mareap->ma_logicstart ||
   (((mstat[mi + 1].md_phyadrs.paf_padrs << 12) + (1UL << 12)) - 1) > mareap->ma_logicend)
  {
   return 0;
  }
  idx++;
...
bool_t check_one_bafhlst(bafhlst_t *bafhp, memarea_t *mareap)
{
 ...
 list_h_t *tmplst = 0;
 msadsc_t *msap = 0;
 for (tmplst = (&bafhp->af_frelst)->next; tmplst != (&bafhp->af_frelst); tmplst = tmplst->next)
 {
  msap = ((msadsc_t *)((char *)(tmplst) - (unsigned long)(&((msadsc_t *)0)->md_list)));
...
bool_t check_one_memarea(memarea_t *mareap)
{
 ...
 for (uint_t li = 0; li < 52; li++)
 {
  if (check_one_bafhlst(&mareap->ma_mdmdata.dm_mdmlielst[li], mareap) == 0)
  {
   return 0;
  }
 }

 if (check_one_bafhlst(&mareap->ma_mdmdata.dm_onemsalst, mareap) == 0)
 {
  return 0;
 }
 return 1;
...
void mem_check_mareadata(machbstart_t *mbsp)
{
 ...
 {
  if (check_one_memarea(&marea[maidx]) == 0)
  {
...
void init_merlove_mem()
{
 if (merlove_mem_core(&kmachbsp) == 0)
 {
