...
void vsprintfk(char_t *buf, const char_t *fmt, va_list args)
{
 ...
 while (*fmt)
 {
  ...
  {
  case 'x':
   p = numberk(p, __builtin_va_arg(args,uint_t), 16);
   fmt++;
   break;
  case 'd':
   p = numberk(p, __builtin_va_arg(args,uint_t), 10);
   fmt++;
   break;
  case 's':
   p = strcopyk(p, (char_t *)__builtin_va_arg(args,uint_t));
   fmt++;
   break;
  default:
   break;
