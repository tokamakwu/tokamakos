...
void set_descriptor(descriptor_t *p_desc, u32_t base, u32_t limit, u16_t attribute)
{
    ...
    p_desc->attr1 = (u8_t)(attribute & 0xFF);
    p_desc->limit_high_attr2 = (u8_t)(((limit >> 16) & 0x0F) | ((attribute >> 8) & 0xF0));
 ...
 void load_x64_tr(u16_t trindx)
{
 ...
 void init_descriptor()
{
    for (u32_t gdtindx = 0; gdtindx < 1; gdtindx++)
    {
        set_descriptor(&x64_gdt[gdtindx][0], 0, 0, 0);
        set_descriptor(&x64_gdt[gdtindx][1], 0, 0, 0x9A | 0x2000 | 0);
        set_descriptor(&x64_gdt[gdtindx][2], 0, 0, 0x92 | 0x2000 | 0);
        set_descriptor(&x64_gdt[gdtindx][3], 0, 0, 0x9A | 0x2000 | 0x60 | 0);
        set_descriptor(&x64_gdt[gdtindx][4], 0, 0, 0x92 | 0x2000 | 0x60 | 0);
        set_x64tss_descriptor(&x64_gdt[gdtindx][6], (u64_t)&x64tss[gdtindx], sizeof(x64tss[gdtindx]) - 1, 0x89);
 ...
 void init_idt_descriptor()
{
    ...
    {
        set_idt_desc((u8_t)intindx, 0x8E, hxi_exc_general_intpfault, 0);
    }

    set_idt_desc(0x0, 0x8E, exc_divide_error, 0);

    set_idt_desc(0x1, 0x8E, exc_single_step_exception, 0);

    set_idt_desc(0x2, 0x8E, exc_nmi, 0);

    set_idt_desc(0x3, 0x8E, exc_breakpoint_exception, 3);

    set_idt_desc(0x4, 0x8E, exc_overflow, 3);

    set_idt_desc(0x5, 0x8E, exc_bounds_check, 0);

    set_idt_desc(0x6, 0x8E, exc_inval_opcode, 0);

    set_idt_desc(0x7, 0x8E, exc_copr_not_available, 0);

    set_idt_desc(0x8, 0x8E, exc_double_fault, 0);

    set_idt_desc(0x9, 0x8E, exc_copr_seg_overrun, 0);

    set_idt_desc(0xA, 0x8E, exc_inval_tss, 0);

    set_idt_desc(0xB, 0x8E, exc_segment_not_present, 0);

    set_idt_desc(0xC, 0x8E, exc_stack_exception, 0);

    set_idt_desc(0xD, 0x8E, exc_general_protection, 0);

    set_idt_desc(0xE, 0x8E, exc_page_fault, 0);



    set_idt_desc(0x10, 0x8E, exc_copr_error, 0);

    set_idt_desc(0x11, 0x8E, exc_alignment_check, 0);

    set_idt_desc(0x12, 0x8E, exc_machine_check, 0);

    set_idt_desc(0x13, 0x8E, exc_simd_fault, 0);

    set_idt_desc(0x20 + 0, 0x8E, hxi_hwint00, 0);

    set_idt_desc(0x20 + 1, 0x8E, hxi_hwint01, 0);

    set_idt_desc(0x20 + 2, 0x8E, hxi_hwint02, 0);

    set_idt_desc(0x20 + 3, 0x8E, hxi_hwint03, 0);

    set_idt_desc(0x20 + 4, 0x8E, hxi_hwint04, 0);

    set_idt_desc(0x20 + 5, 0x8E, hxi_hwint05, 0);

    set_idt_desc(0x20 + 6, 0x8E, hxi_hwint06, 0);

    set_idt_desc(0x20 + 7, 0x8E, hxi_hwint07, 0);

    set_idt_desc(0x28 + 0, 0x8E, hxi_hwint08, 0);

    set_idt_desc(0x28 + 1, 0x8E, hxi_hwint09, 0);

    set_idt_desc(0x28 + 2, 0x8E, hxi_hwint10, 0);

    set_idt_desc(0x28 + 3, 0x8E, hxi_hwint11, 0);

    set_idt_desc(0x28 + 4, 0x8E, hxi_hwint12, 0);

    set_idt_desc(0x28 + 5, 0x8E, hxi_hwint13, 0);

    set_idt_desc(0x28 + 6, 0x8E, hxi_hwint14, 0);

    set_idt_desc(0x28 + 7, 0x8E, hxi_hwint15, 0);

    set_idt_desc(0x28 + 8, 0x8E, hxi_hwint16, 0);

    set_idt_desc(0x28 + 9, 0x8E, hxi_hwint17, 0);

    set_idt_desc(0x28 + 10, 0x8E, hxi_hwint18, 0);

    set_idt_desc(0x28 + 11, 0x8E, hxi_hwint19, 0);

    set_idt_desc(0x28 + 12, 0x8E, hxi_hwint20, 0);

    set_idt_desc(0x28 + 13, 0x8E, hxi_hwint21, 0);

    set_idt_desc(0x28 + 14, 0x8E, hxi_hwint22, 0);

    set_idt_desc(0x28 + 15, 0x8E, hxi_hwint23, 0);

    set_idt_desc(0xe0, 0x8E, hxi_apic_ipi_schedul, 0);

    set_idt_desc(0xef, 0x8E, hxi_apic_svr, 0);

    set_idt_desc(0xf0, 0x8E, hxi_apic_timer, 0);

    set_idt_desc(0xf1, 0x8E, hxi_apic_thermal, 0);

    set_idt_desc(0xf2, 0x8E, hxi_apic_performonitor, 0);

    set_idt_desc(0xf3, 0x8E, hxi_apic_lint0, 0);

    set_idt_desc(0xf4, 0x8E, hxi_apic_lint1, 0);

    set_idt_desc(0xf5, 0x8E, hxi_apic_error, 0);

    set_idt_desc(0xFF, 0x8E, exi_sys_call, 3);
