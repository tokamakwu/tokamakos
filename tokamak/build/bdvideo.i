 ...
 void init_dftgraph()
{
    ...
    kghp->gh_fnthight = 16;
    kghp->gh_linesz = 16 + 4;
    kghp->gh_deffontpx = ((0 | (0xff << 16) | (0xff << 8) | 0xff));
    return;
}
...
void hal_dspversion()
{
    pixl_t bkpx = set_deffontpx(((0 | (0xff << 16) | (0 << 8) | 0)));
...
void fill_fullbitmap(char_t *bmfname)
{
    ...
    for (int j = 768; j >= 0; j--, l++)
    {
        ...
        {
            pix = ((0 | (bmdp[k].bmd_r << 16) | (bmdp[k].bmd_g << 8) | bmdp[k].bmd_b));
...
sint_t bga_set_xy(void *ghpdev, uint_t x, uint_t y)
{
    bga_write_reg((1), (u16_t)(x));
    bga_write_reg((2), (u16_t)(y));
...
sint_t bga_set_vwh(void *ghpdev, uint_t vwt, uint_t vhi)
{
    bga_write_reg((6), (u16_t)vwt);
    bga_write_reg((7), (u16_t)vhi);
    return 0;
}

sint_t bga_set_xyoffset(void *ghpdev, uint_t xoff, uint_t yoff)
{
    bga_write_reg((8), (u16_t)(xoff));
    bga_write_reg((9), (u16_t)(yoff));
    return 0;
}
...
sint_t bga_get_vwh(void *ghpdev, uint_t *rvwt, uint_t *rvhi)
{
    ...
    u16_t retwt, rethi;
    retwt = bga_read_reg((6));
    rethi = bga_read_reg((7));
    *rvwt = (uint_t)retwt;
    *rvhi = (uint_t)rethi;
    return 0;
}
...
sint_t bga_get_xyoffset(void *ghpdev, uint_t *rxoff, uint_t *ryoff)
{
    ...
    retxoff = bga_read_reg((8));
    retyoff = bga_read_reg((9));
