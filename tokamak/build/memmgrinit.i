...
void init_memmgrob()
{
 ...
 if (0 == mbsp->mb_e820expadr || 0 == mbsp->mb_e820exnr)
 {
  system_error("mbsp->mb_e820expadr==NULL\n");
 }
 if (0 == mbsp->mb_memmappadr || 0 == mbsp->mb_memmapnr)
 {
  system_error("mbsp->mb_memmappadr==NULL\n");
 }
 if (0 == mbsp->mb_memznpadr || 0 == mbsp->mb_memznnr)
 {
 ...
 mobp->mo_memsz = mbsp->mb_memmapnr << 12;
 mobp->mo_maxpages = mbsp->mb_memmapnr;
 uint_t aidx = 0;
 for (uint_t i = 0; i < mobp->mo_msanr; i++)
 {
  if (1 == mobp->mo_msadscstat[i].md_indxflgs.mf_uindx &&
   (1) == mobp->mo_msadscstat[i].md_indxflgs.mf_mocty &&
   (1) == mobp->mo_msadscstat[i].md_phyadrs.paf_alloc)
  {
   aidx++;
  }
 }
...
void memmgrob_t_init(memmgrob_t *initp)
{
 ...
 initp->mo_pmagestat = 0;
 initp->mo_pmagenr = 0;
 initp->mo_msadscstat = 0;
 initp->mo_msanr = 0;
 initp->mo_mareastat = 0;
 initp->mo_mareanr = 0;
 initp->mo_privp = 0;
 initp->mo_extp = 0;
 return;
...
bool_t copy_pages_data(machbstart_t *mbsp)
{
 ...
 for (uint_t pdei = 0; pdei < 16; pdei++)
 {
  ...
  pdpte[pdei] = (uint_t)(pdepd | (1 << 1) | (1 << 0));
 ...
 p[((0xffff800000000000) >> 39) & 0x1ff] = (uint_t)(pdptepd | (1 << 1) | (1 << 0));
 p[0] = (uint_t)(pdptepd | (1 << 1) | (1 << 0));
 mbsp->mb_pml4padr = topgadr;
 mbsp->mb_subpageslen = (uint_t)(0x1000 * 16 + 0x2000);
 mbsp->mb_kpmapphymemsz = (uint_t)(0x400000000);
 mbsp->mb_nextwtpadr = (((mbsp->mb_pml4padr + mbsp->mb_subpageslen) + ((1UL << 12))-1) & ~(((1UL << 12))-1));
 return 1;
}
...
bool_t copy_fvm_data(machbstart_t *mbsp, dftgraph_t *dgp)
{
 ...
 mbsp->mb_nextwtpadr = (((tofvadr + dgp->gh_fvrmsz) + ((1UL << 12))-1) & ~(((1UL << 12))-1));
 return 1;
}
...
void init_copy_pagesfvm()
{
 if (copy_pages_data(&kmachbsp) == 0)
 {
  system_error("copy_pages_data fail");
 }
 if (copy_fvm_data(&kmachbsp, &kdftgh) == 0)
 {
  system_error("copy_fvm_data fail");
 }
 memi_set_mmutabl(kmachbsp.mb_pml4padr, 0);
 return;
}
