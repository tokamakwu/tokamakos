...
# 7 "../hal/x86/msadsc.c" 2

void msadsc_t_init(msadsc_t *initp)
{
 list_init(&initp->md_list);
 knl_spinlock_init(&initp->md_lock);
 initp->md_indxflgs.mf_olkty = (0);
 initp->md_indxflgs.mf_lstty = (0);
 initp->md_indxflgs.mf_mocty = (0);
 initp->md_indxflgs.mf_marty = (0);
 initp->md_indxflgs.mf_uindx = (0);
 initp->md_phyadrs.paf_alloc = (0);
 initp->md_phyadrs.paf_shared = (0);
 initp->md_phyadrs.paf_swap = (0);
 initp->md_phyadrs.paf_cache = (0);
 initp->md_phyadrs.paf_kmap = (0);
 initp->md_phyadrs.paf_lock = (0);
 initp->md_phyadrs.paf_dirty = (0);
 initp->md_phyadrs.paf_busy = (0);
 initp->md_phyadrs.paf_rv2 = (0);
 initp->md_phyadrs.paf_padrs = (0);
 initp->md_odlink = 0;
 return;
}
...
bool_t ret_msadsc_vadrandsz(machbstart_t *mbsp, msadsc_t **retmasvp, u64_t *retmasnr)
{
 if (0 == mbsp || 0 == retmasvp || 0 == retmasnr)
 {
  return 0;
 }
 if (mbsp->mb_e820exnr < 1 || 0 == mbsp->mb_e820expadr || (mbsp->mb_e820exnr * sizeof(phymmarge_t)) != mbsp->mb_e820exsz)
 {
  *retmasvp = 0;
  *retmasnr = 0;
  return 0;
...
void init_msadsc()
{
 ...
 if (ret_msadsc_vadrandsz(mbsp, &msadscvp, &msadscnr) == 0)
 {
 ...
 mbsp->mb_nextwtpadr = (((mbsp->mb_memmappadr + mbsp->mb_memmapsz) + ((1UL << 12))-1) & ~(((1UL << 12))-1));
...
u64_t search_segment_occupymsadsc(msadsc_t *msastart, u64_t msanr, u64_t ocpystat, u64_t ocpyend)
{
 ...
 {
  if ((msastart[mnr].md_phyadrs.paf_padrs << 12) == ocpystat)
  {
...
step1:
 ...
 for (u64_t tmpadr = ocpystat; tmpadr < ocpyend; tmpadr += (1UL << 12), fsmsnr++)
 {

  mphyadr = fstatmp[fsmsnr].md_phyadrs.paf_padrs << 12;
  if (mphyadr != tmpadr)
  {
   return 0;
  }
  if ((0) != fstatmp[fsmsnr].md_indxflgs.mf_mocty ||
   0 != fstatmp[fsmsnr].md_indxflgs.mf_uindx ||
   (0) != fstatmp[fsmsnr].md_phyadrs.paf_alloc)
  {
   return 0;
  }


  fstatmp[fsmsnr].md_indxflgs.mf_mocty = (1);
  fstatmp[fsmsnr].md_indxflgs.mf_uindx++;
  fstatmp[fsmsnr].md_phyadrs.paf_alloc = (1);
 }
...
void test_schkrloccuymm(machbstart_t *mbsp, u64_t ocpystat, u64_t sz)
{
 ...
 for (u64_t mnr = 0; mnr < msanr; mnr++)
 {
  if ((msadstat[mnr].md_phyadrs.paf_padrs << 12) == ocpystat)
  {
...
step1:
 ...
 for (u64_t i = 0; i < chkmnr; i++, chkadr += (1UL << 12))
 {
  if (chkadr != fstatmp[i].md_phyadrs.paf_padrs << 12)
  {
   system_error("chkadr != err\n");
  }
  if ((1) != fstatmp[i].md_phyadrs.paf_alloc)
  {
  ...
  if ((1) != fstatmp[i].md_indxflgs.mf_mocty)
  {
   system_error("mf_olkty err\n");
  }
 }
 if (chkadr != (ocpystat + (chkmnr * (1UL << 12))))
 {
...
void init_search_krloccupymm(machbstart_t *mbsp)
{
 ...
 if (search_krloccupymsadsc_core(mbsp) == 0)
 {
